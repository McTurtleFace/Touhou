...
void spawnLoop(int currentBoss, QElapsedTimer * timer, vector<Visual *> * visuals, vector<Visual *> * newVisuals, Sprite * spriteList[NUMSPRITES]) {
    ...
    switch(currentBoss){
    case 1:{
        static uint32_t currentStage = 0;
...
DWORD WINAPI threadedMain(LPVOID lpParam){
...
int main(int argc, char *argv[])
{
    ...
    struct ThreadParams params = { argc, argv };
    DWORD threadId;
    HANDLE hThread = CreateThread(NULL, 64 * 1024 * 1024, threadedMain, &params, 0, &threadId);
    if (hThread == NULL) {
